...
def alert_driver():
    ...
    while wake_up:
        mixer.init()
        mixer.music.load('/home/compiler/PycharmProjects/Driver drowsiness detection/al1.mp3')
        mixer.music.play()
        time.sleep(2)
...
def start_monitoring():
    ...
    while True:
        ...
        cv2.putText(frame, "EAR value : {:.4f}".format(detected_ear), (25, 170), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    (25, 111, 130), 2)
...
if __name__ == "__main__":
    drowsy_ear = compute_drowsy_ear(spectacles=True)
    # Normal EAR relative to the driver's posture, eye shape and eye spectacles detection
    set_thresholds(eye_aspect_ratio=drowsy_ear, frame_threshold=20)
    # Greater EAR or lesser frame threshold = Greater sensitivity
    create_detector()
    # Detector
    load_predictor(predictor_path="face.dat")
    # Predictor
    fetch_eye_landmarks()
    # Eye landmarks
    start_monitoring()
